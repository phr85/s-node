...
<input type="hidden" name="x{$BASEID}_action" value="saveRole">
<input type="submit" value="{'Save role'|translate}" name="submit" class="button">
<br><br>
<table cellspacing="0" cellpadding="0" width="100%">
 <tr>
  <td class="table_header" colspan="2">{"Role data"|translate}</td>
 </tr>
 <tr>
...
<br>
<form method="POST" name="addUserToRole">
<input type="hidden" name="x{$BASEID}_action" value="">
<table cellspacing="0" cellpadding="0" width="100%">
 <tr>
  <td class="table_header" colspan="2">{"Add user to this role"|translate}</td>
 </tr>
 <tr>
  <td class="left">{"Username"|translate}</td>
  <td class="right"><input type="text" size="42" name="x{$BASEID}_username">&nbsp;<input type="submit" class="button" value="Add user to role" onclick="document.forms['addUserToRole'].x{$BASEID}_action.value='addUserToRole'"></td>
 </tr>
</table>
</form>
<br>
 ...
 <table cellspacing="0" cellpadding="0" width="100%">
  ...
  {foreach from=$DATA item=USER name=USERTABLE}
      <tr class="{cycle values="row_a,row_b"}">
       <td class="button">
       {if "edit"|allowed}<a href="javascript:ask('{'Are you sure to remove user from role?'|translate}','index.php?TPL={$TPL}&x{$BASEID}_action=deleteUserFromRole&x{$BASEID}_user_id={$USER.id}');"><img src="images/icons/delete.png" alt="{'Delete this user'|translate}" title="{'Delete this user'|translate}"></a>{else}{$ICONSPACER}{/if}<br>
       </td>
