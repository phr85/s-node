 ...
 <table cellspacing="0" cellpadding="0" width="100%">
  ...
  {foreach from=$USERS item=USER name=USERTABLE}
      <tr class="{cycle values="row_a,row_b"}" {if $ACTIVE_PERM_ID == $USER.id}style="background-color: #6E8297;"{/if}>
       <td class="button">
       {if "managePagePermissions"|allowed}<a href="javascript:ask('{'Are you sure to remove this users permissions?'|translate}','index.php?TPL={$TPL}&x{$BASEID}_action=removePagePermission&x{$BASEID}_perm_id={$USER.id}');"><img src="images/icons/delete.png" alt="{'Delete this permission'|translate}" title="{'Delete this permissions'|translate}"></a>{else}{$ICONSPACER}{/if}<br>
       </td>
