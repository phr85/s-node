...
if(XT::getPermission('list')){
    ...
    if(XT::getPermission('browserStructure')){
        if($GLOBALS['plugin']->getSessionValue('ctrl_add') || $GLOBALS['plugin']->getSessionValue('ctrl_cut') || $GLOBALS['plugin']->getSessionValue('ctrl_copy')){
            ...
            (sizeof($count) > 0) ? $GLOBALS['plugin']->contribute('browser_buttons','Add node', 'addNode','document_new.png','1','master') : $GLOBALS['plugin']->contribute('browser_buttons','Add node', 'addFirstNode','document_new.png','1','master');;
        }
    }
