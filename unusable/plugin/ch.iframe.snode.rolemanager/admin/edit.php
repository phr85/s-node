...
if($GLOBALS['plugin']->getSessionValue('id') && XT::getPermission('edit')){
    ...
    $result = XT::query("SELECT id,title,description,active,creation_date,creation_user,mod_date,mod_user FROM " . $GLOBALS['plugin']->getTable("roles") . " WHERE id = " . $GLOBALS['plugin']->getSessionValue("id") . "",__FILE__,__LINE__);
    ...
    XT::enableAdminNavigator('user','',"
        SELECT
            count(b.id)
        FROM
            " . $GLOBALS['plugin']->getTable("user_roles") . " as a,
