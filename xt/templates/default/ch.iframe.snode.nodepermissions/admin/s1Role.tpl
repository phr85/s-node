...
{/if}
{include file="includes/lang_selector_submit.tpl" form="slave1" action="s1SaveRolePermission"}
<table cellspacing="0" cellpadding="0" width="100%">
...
<tr class="{cycle values="row_a,row_b"}">
 ...
 <td class="row">{if $PERM.rights}<span style="color: green;">{$PERM.perm|translate}</span>{else}<span style="color: red;">{$PERM.perm|translate}</span>{/if}</td>
</tr>
{/foreach}
</table>