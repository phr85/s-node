...
<td class="lang_tab">{"Usermode"|translate}</td>
</table>
<table cellspacing="0" cellpadding="0" width="100%">
...
<td class="row" style="padding-left: 5px; width: 1px;padding-right: 0px;"><img src="{$XT_IMAGES}icons/user1.png" width="16" /></td>
      <td class="row"> {if $USER.id == $USER_ID}
      {actionLink
      action= "clickOnUser"
      form="master"
      yoffset=1
      ...
      {if $USER.lastName || $USER.firstName}({$USER.firstName}{if $USER.lastName} {$USER.lastName}{/if}){/if}
