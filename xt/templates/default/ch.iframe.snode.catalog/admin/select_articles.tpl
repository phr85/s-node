...
<table cellspacing="0" cellpadding="0" width="100%">
     <tr>
      ...
      <tr class="{cycle values="row_a,row_b"}">
       <td class="button">
       {if $ENTRY.article_id == $ENTRY.id}
       {if "edit"|allowed}<a href="{$smarty.server.PHP_SELF}?TPL={$TPL}&x{$BASEID}_action=removeArticleFromTree&x{$BASEID}_article_id={$ENTRY.id}&x{$BASEID}_node_id={$ENTRY.node_id}"><img src="images/icons/check.png" alt="{"add"|translate}" title="{"add"|translate}" /></a>{/if}
       {else}
       {if "edit"|allowed}<a href="{$smarty.server.PHP_SELF}?TPL={$TPL}&x{$BASEID}_action=addArticleToTree&x{$BASEID}_id={$ENTRY.id}"><img src="images/icons/folder_add.png" alt="{"add"|translate}" title="{"add"|translate}" /></a>{/if}
       {/if}
       </td>
       <td class="row">
       {$ENTRY.id}
