    ...
    <table cellspacing="0" cellpadding="0" width="100%">
     <tr>
      ...
      <td class="row"><a href="javascript:window.document.forms[0].x{$BASEID}_open.value={$NODE.id};window.document.forms[0].submit();document.forms['o'].x{$BASEID}_open.value={$NODE.id};document.forms['o'].submit();">{if $NODE.itw}<span style="color: black;">{if $NODE.selected}<b>{$NODE.title}</b>{else}{$NODE.title}{/if}</span>{else}{$NODE.title}{/if}&nbsp;</a></td>
       ...
       <td class="button" width="80" align="right"><a href="index.php?TPL={$TPL}&x{$BASEID}_lang={$ACTIVE_LANG}&x{$BASEID}_insertPage={$INSERTPAGE}&x{$BASEID}_category=0,{$NODE.id}&x{$BASEID}_target_node_id={$TARGETNODEID}"><img src="images/icons/check.png" {"select this article"|alttag}></a></td>
   ...
   <td>
    <table cellspacing="0" cellpadding="0" width="100%">
     <tr class="{cycle values="row_a,row_b"}">
      ...
      {else}{$ARTICLE.title}{if $ARTICLE.subs > 0}&nbsp;({$ARTICLE.subs}){/if}{/if}{if !$ARTICLE.lang_na}{/if}
...
<input type="hidden" name="x{$BASEID}_insertPage" value="{$INSERTPAGE}" />
<input type="hidden" name="x{$BASEID}_id" value="" />
<input type="hidden" name="x{$BASEID}_position" value="" />
<input type="hidden" name="x{$BASEID}_action" value="" />
<input type="hidden" name="x{$BASEID}_node_id" value="" />
<input type="hidden" name="x{$BASEID}_target_node_id" value="{$TARGETNODEID}" />
