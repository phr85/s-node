...
<a href="{$smarty.server.PHP_SELF}?TPL={$TPL}&amp;x{$BASEID}_view={$IMAGE.id}&amp;x{$BASEID}_id={$GALLERY.id}">
{image
    id=$IMAGE.id
    ...
    <p>{if $SHOW_TITLES}<a href="{$smarty.server.PHP_SELF}?TPL={$TPL}&amp;x{$BASEID}_view={$IMAGE.id}&amp;x{$BASEID}_id={$GALLERY.id}"><b>{$IMAGE.title}</b></a><br />{/if}{if $SHOW_VIEWS}{$IMAGE.views} {"Views"|translate}{/if}</p></div>
