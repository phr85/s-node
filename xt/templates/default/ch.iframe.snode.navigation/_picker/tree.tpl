   ...
   <td>
    <table cellspacing="0" cellpadding="0" width="100%">
     <tr>
      ...
      <td class="row">{if !$NODE.lang_na}<a href="#" onClick="javascript:window.parent.frames['slave1'].location.href='{$smarty.server.PHP_SELF}?TPL={$NODE.id}&amp;tmp_lang={$ACTIVE_LANG}';{if $NODE.subs != 0}javascript:document.forms['navigation'].x{$BASEID}_yoffset.value=window.pageYOffset;document.forms['navigation'].x{$BASEID}_open.value={$NODE.id};document.forms['navigation'].submit();{/if}">{/if}{if $NODE.itw}<span style="color: black;">{if $NODE.selected}<b>{$NODE.title}</b>{if $NODE.subs > 0}&nbsp;({$NODE.subs}){/if}{else}{$NODE.title}{if $NODE.subs > 0}&nbsp;({$NODE.subs}){/if}{/if}</span>{else}{$NODE.title}{if $NODE.subs > 0}&nbsp;({$NODE.subs}){/if}{/if}{if !$NODE.lang_na}</a>{/if}</td>
      <td class="button" align="right">
<a href="javascript:
{if $MODE != "full"}
window.parent.opener.document.forms['{$FORM}'].{$FIELD}.value={$NODE.id};
{else}
window.parent.opener.document.forms['{$FORM}'].{$FIELD}.value='{$smarty.server.PHP_SELF}?TPL={$NODE.id}';
{/if}

{if $TITLEFIELD == ""}
window.parent.opener.document.forms['{$FORM}'].{$FIELD}_title.value='{$NODE.title|cleanlink}';
{else}
window.parent.opener.document.forms['{$FORM}'].{$TITLEFIELD}.value='{$NODE.title|cleanlink}';
{/if}
