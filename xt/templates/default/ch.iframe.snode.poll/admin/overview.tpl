<form method="POST" name="poll">
 ...
 <table cellspacing="0" cellpadding="0" width="100%">
  <tr>
   ...
   <td class="table_header" width="100">{actionIcon action="NULL" form=poll label="Date" sort=$sort.0.value icon=$sort.0.icon}</td>
   <td class="table_header" width="40">{actionIcon action="NULL" label="ID" form=poll sort=$sort.1.value icon=$sort.1.icon}</td>
   <td class="table_header">{actionIcon action="NULL" form=poll label="Title" sort=$sort.2.value icon=$sort.2.icon}</td>
  </tr>
  ...
  <tr class="{cycle values="row_a,row_b"}" {if $POLL.locked_user == $USER_ID}style="background-image: url({$XT_IMAGES}admin/gfx/naventry_active.gif);"{/if}>
   ...
   <td class="row">{$POLL.date|date_format:"%d.%m.%Y %H:%I"}</td>
