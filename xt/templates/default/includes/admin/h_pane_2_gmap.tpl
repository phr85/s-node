    ...
    {include file="includes/header/header_admin.tpl"}
     <table cellspacing="0" cellpadding="0" width="100%" style="height: 100%;">
      <tr>
       ...
       <td>
        {iframe_paper url="?TPL=%TPL%&amp;tabs=0&amp;module=%MODULE%&amp;gmapheader=true" name="slave1" template=$s1_template s1_tab=$s1_tab}
       </td>
      </tr>
     </table>
{include file="includes/footer/footer_admin.tpl"}
{else}
{if $smarty.request.x7000_gmapheader=="true"}
{include file="includes/header/header_admin_empty_gmap.tpl"}
{else}
