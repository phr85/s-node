...
<div class="title">{"Relations"|translate}:</div>
...
Titel :
  ...
  {xt_get_relations cid=$cid ctype=$ctype lang=$ACTIVE_LANG|default:"de"}
  {if $RELATIONS.list|@count > 0}
<div class="title">{"Used Relations"|translate}</div>
...
<div class="row"><div class="l">
	...
	<img src="{$XT_IMAGES}icons/{$RELATION.icon}" alt="{$RELATION.title}"/> {if $RELATION.title == ""}{"unknown"|translate}{else}{$RELATION.title}{/if} 
</div><div class="c"></div></div>
