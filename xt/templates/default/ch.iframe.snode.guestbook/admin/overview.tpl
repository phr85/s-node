<form method="POST" name="guestbook">
 ...
 <table cellspacing="0" cellpadding="0" width="100%">
  <tr>
   ...
   <td class="table_header">{"Date"|translate}</td>
  </tr>
  {foreach from=$DATA item=ENTRY name=ENTRYTABLE}
  <tr class="{cycle values="row_a,row_b"}">
   <td class="button">
   {if "statuschange"|allowed}{if $ENTRY.active == 1}<a href="index.php?TPL={$TPL}&amp;x{$BASEID}_action=deactivateEntry&amp;x{$BASEID}_id={$ENTRY.id}"><img src="images/icons/active.png" alt="{'Status'|translate}" title="{'Activate/Deactivate this entry'|translate}" style="padding-right: 4px;" /></a>{else}<a href="index.php?TPL={$TPL}&amp;x{$BASEID}_action=activateEntry&amp;x{$BASEID}_id={$ENTRY.id}"><img src="images/icons/inactive.png" style="padding-right: 4px;" /></a>{/if}{else}{$ICONSPACER}{/if}{if "edit"|allowed}<a href="index.php?TPL={$TPL}&amp;x{$BASEID}_action=editEntry&amp;x{$BASEID}_id={$ENTRY.id}"><img src="images/icons/pencil.png" alt="{'Edit'|translate}" title="{'Edit this ENTRY'|translate}" style="padding-right: 4px;" /></a>{else}{$ICONSPACER}{/if}{if "delete"|allowed}<a href="javascript:ask('Are you sure you want to delete this entry ?','{$smarty.server.PHP_SELF}?TPL={$TPL}&amp;x{$BASEID}_action=deleteEntry&amp;x{$BASEID}_id={$ENTRY.id}');"><img src="images/icons/delete.png" alt="{'Delete'|translate}" title="{'Delete this ENTRY'|translate}" ></a>{else}{$ICONSPACER}{/if}<br />
   </td>
