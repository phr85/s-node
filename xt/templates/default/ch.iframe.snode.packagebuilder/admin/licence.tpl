...
<tr>
 ...
 <select name=x{$BASEID}_value[product][]" style="width:360px;" size="15" multiple/>
 {foreach from=$PRODUCTS item=PRODUCT}
      <option label="{$PRODUCT}" value="{$PRODUCT}" {if $SELECTEDPRODUCTS[$PRODUCT]}selected="selected"{/if}>{$PRODUCT}</option>  
 {/foreach}
 </select>{actionIcon form="licence" action="selectMini" icon="check.png" title="Select Mini" label="Select Mini"} 
 {actionIcon form="licence" action="selectAll" icon="check.png" title="Select All" label="Select All"} 
 </td>
</tr>
<tr>
 <td class="left">
...
<tr>
 ...
 <td class="right">
  <input type="text" name="x{$BASEID}_value[limits]" value="{$VALUES.limits}">
 </td>
</tr>
</table>
</form>
