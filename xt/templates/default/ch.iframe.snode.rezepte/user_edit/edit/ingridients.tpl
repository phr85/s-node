...
{foreach from=$INGRIDIENTS name=ING item=INGRIDIENT}
 <tr>
  <td class="create_left{if $INGRIDIENT.unit_id==17} highlight{/if}">{actionIcon 
  action="removeIngridientFromRecipe" title="remove" yoffset="1" icon="delete.png" form="create" ingridient_id=$INGRIDIENT.id}{if !$smarty.foreach.ING.last}{actionIcon 
  action="moveIngridientInRecipeDown" title="move down" yoffset="1" icon="explorer/arrow_down_green.png" form="create" ingridient_id=$INGRIDIENT.id position=$INGRIDIENT.position}{else}{$ICONSPACER}{/if}{if !$smarty.foreach.ING.first}{actionIcon 
  action="moveIngridientInRecipeUp" title="move up" yoffset="1" icon="explorer/arrow_up_green.png" form="create" ingridient_id=$INGRIDIENT.id position=$INGRIDIENT.position}{/if}
  </td>
  <td class="create_right{if $INGRIDIENT.unit_id==17} highlight{/if}">
...
{literal}
var options = {
    script: "index.php?TPL=10077&ctype=xml&json=true&",
    varname: "input",
    minchars: 3,
    shownoresults:false,
    json:true,
...
{/foreach}
 ...
 <tr>
  <td class="create_left lightgrey">
 {"new"|translate}
  </td>
  <td class="create_right lightgrey">
