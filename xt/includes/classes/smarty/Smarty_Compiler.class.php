...
/* $Id: Smarty_Compiler.class.php 3499 2007-07-26 09:15:50Z mgraf $ */
...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_include_tag($tag_args)
    {
        ...
        $this->secure_dir[0] = ROOT_DIR . 'templates/' . $_SESSION['theme'] . '/';
        if($_SESSION['theme'] != 'default'){
            if(is_file(ROOT_DIR . 'templates/' . $_SESSION['theme'] . '/' . str_replace('"','',$include_file))){
                $include_file = '"' . ROOT_DIR . 'templates/' . $_SESSION['theme'] . '/' . str_replace('"','',$include_file) . '"';
            }
        }
            ...
            foreach ($_indexes as $_index) {
                if ($_index{0} == '[') {
                    ...
                    if(substr($_index,2,2) == '__') {
                        ...
                        $this->_syntax_error('(secure) call to var object member is not allowed', E_USER_ERROR, __FILE__, __LINE__);
                    } elseif ($_index{2} == '$') {
                        if ($this->security) {
