...
if(XT::getPermission('add')){
    // Add action buttons
    XT::addButton('Save', 'saveArticleTime');
}
...
if(is_numeric($GLOBALS['plugin']->getSessionValue('id'))){
    ...
    // Get the article timesettings out of the table
    $result = XT::query("
       SELECT
           time_active,
           time_r_active,
           ...
           time_r_end
       FROM
           " . $GLOBALS['plugin']->getTable("article") . "
       WHERE
    ...
    $result = XT::query("SELECT * FROM " . $GLOBALS['plugin']->getTable("article") .
