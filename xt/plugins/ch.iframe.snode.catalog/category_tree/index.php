...
$opennode_id = XT::autoval("node","R",XT::getParam("node"));
...
if ($GLOBALS['plugin']->getParam("follow") == 1){
    // get follow details
    $result = XT::query("SELECT title, id, l, r, pid, level FROM " . $GLOBALS['plugin']->getTable("tree") . " WHERE id='" . XT::getSessionValue('node') . "'" ,__FILE__,__LINE__);
    $follow = array();
...
if($in != '' && !$no_nodes){
    $result = XT::query("
            SELECT
                ...
                count(article.article_id) as elements,
            ...
            FROM
                " . $GLOBALS['plugin']->getTable("tree") . " AS main
            LEFT JOIN
                " . $GLOBALS['plugin']->getTable("nodes") . " AS details ON (details.node_id = main.ID AND details.lang = '" . $lang . "')
            LEFT JOIN
                " . $GLOBALS['plugin']->getTable("tree2articles") . " AS article ON (article.node_id = main.id)
            WHERE "
    . $sql['open_depth'] . "
    ( main.pid in (" . $in . ")"
    . $sql['start_level']
