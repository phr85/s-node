...
if(!XT::getValue('recipe_id')){
    XT::setValue('recipe_id', XT::getSessionValue('recipe_id'));
}else{
    XT::setSessionValue('recipe_id',XT::getValue('recipe_id'));
}
...
if (XT::getValue('recipe_id') == '') {
    if(XT::getSessionValue('recipe_id') == '') {
        XT::setSessionValue('recipe_id', 0);
    }
}
...
if($node_id == 1 || empty($node_id)){
    ...
    $result = XT::query("
        SELECT
            ap.node_id
        FROM
            " . XT::getTable("r2tree") . " as ap
        WHERE
            ap.recipe_id = " . XT::getSessionValue('recipe_id') . "
        LIMIT 1
...
if(XT::getValue('recipe_id') || XT::getSessionValue('recipe_id')){
    $result = XT::query("
        SELECT
            ap.node_id,
            ap.recipe_id,
        ...
        FROM
            ...
            " . XT::getTable("r_details") . " as ad
        WHERE
            ap.node_id = " . XT::getSessionValue('node') . "
            AND (ap.recipe_id = ad.id AND ad.lang='" . $GLOBALS['lang']->getLang() . "' AND ad.active = 1)
        ORDER by
            ap.position asc
    ",__FILE__,__LINE__,XT::getParam("debug_sql"));
    $position = 1;
    while($row = $result->FetchRow()){
        ...
        $PRODUCTS[$position]['recipe_id'] = $row['recipe_id'];
        ...
        if($row['recipe_id'] == XT::getSessionValue('recipe_id')) {
    ...
    if(is_array($PRODUCTS)){
        ...
        $content = 'required value (recipe_id) not available';
    }
}
?>
