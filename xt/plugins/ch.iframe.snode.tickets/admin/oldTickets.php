...
if ($data['client_filter'] > 0) {
	XT::enableAdminNavigator('tickets','id',"SELECT count(*) as count_id FROM " . XT::getTable('tickets') . " WHERE worker=" . XT::getUserId() . " and status > 3 AND client_id=" . $data['client_filter'] );	
	$result = XT::query("SELECT
       *
    FROM
        " . XT::getTable('tickets') . "
	WHERE
		worker=" . XT::getUserId() . " and status > 3 AND
		client_id=" . $data['client_filter'] . "
    ORDER BY
	...
	XT::enableAdminNavigator('tickets','id',"SELECT count(*) as count_id FROM " . XT::getTable('tickets') . " WHERE worker=" . XT::getUserId() . " and status > 3");
	$result = XT::query("SELECT
       *
    FROM
        " . XT::getTable('tickets') . "
...
if ($data['client_filter'] > 0) {
	XT::enableAdminNavigator('tickets','id',"SELECT count(*) as count_id FROM " . XT::getTable('tickets') . " WHERE supervisor=" . XT::getUserId() . " and status > 3 AND client_id=" . $data['client_filter']);
	$result = XT::query("SELECT
       *
    FROM
        " . XT::getTable('tickets') . "
	WHERE
		supervisor=" . XT::getUserId() . " and status > 3 AND
	client_id=" . $data['client_filter'] . "
    ORDER BY
        " . $data['order_by'] . " " . $data['order_by_dir'] . "
    LIMIT
	...
	XT::enableAdminNavigator('tickets','id',"SELECT count(*) as count_id FROM " . XT::getTable('tickets') . " WHERE supervisor=" . XT::getUserId() . " and status > 3");
	$result = XT::query("SELECT
       *
    FROM
        " . XT::getTable('tickets') . "
	WHERE
		supervisor=" . XT::getUserId() . " and status > 3
    ORDER BY
        " . $data['order_by'] . " " . $data['order_by_dir'] . "
    LIMIT
        " . XT::getAdminNavigatorLimit() . "
