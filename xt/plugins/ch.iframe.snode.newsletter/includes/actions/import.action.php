...
if(isset($_FILES['x' . $GLOBALS['plugin']->getBaseID() . '_file']) && $_FILES['x' . $GLOBALS['plugin']->getBaseID() . '_file']['name'] != '' && substr($_FILES['x' . $GLOBALS['plugin']->getBaseID() . '_file']['name'],-4) == '.csv'){
	...
	foreach ($csv->data as $address) {
		...
		if (XT::checkEmail($email)) {
			...
			// Look if the email address already exist
			$result = XT::query("SELECT id FROM " . XT::getTable("newsletter_subscriptions") . " WHERE email='" . $email . "' GROUP by email",__FILE__,__LINE__);
			if ($row = $result->FetchRow()){
				...
				// If the address not exist create a new one
				XT::query("INSERT INTO  " . XT::getTable("newsletter_subscriptions") . "
				(email,creation_date, creation_user, lang, anrede, company, firstname, lastname, mobile)
				VALUES
				('" . $email . "','" . $now . "','" . XT::getUserID() . "','" . $lang . "','" . addslashes($anrede) . "','" . addslashes($company) . "','" . addslashes($firstname) . "','" . addslashes($lastname) . "','" . addslashes($mobile) . "')
				",__FILE__,__LINE__);

				// Get the id of the new entry
				$result = XT::query("SELECT * FROM " . XT::getTable("newsletter_subscriptions") . " WHERE creation_date=" . $now . " AND email='" . $email . "'",__FILE__,__LINE__);
				$row = $result->FetchRow();
				$subscription_id = $row['id'];
			...
			foreach ($categories as $cat) {
				...
				if ($cat != "") {
					if ($kategorien[$cat] != "") {
						...
						",__FILE__,__LINE__);
						// Get the id of the new entry
						$result = XT::query("SELECT * FROM " . XT::getTable("newsletter_categories") . " WHERE creation_date='" . $now . "'",__FILE__,__LINE__,1);
						$row = $result->FetchRow();
						$catid = $row['id'];
            ...
            XT::log("Invalid E-Mail in row {$delivered_addresses}: {$email} ",__FILE__,__LINE__,XT_ERROR);
        }
	}
	XT::log($delivered_addresses . " delivered, " . $valid_addresses . " valid , " . ($delivered_addresses - $valid_addresses) . " NOT imported, " . $updated_addresses . " updated"  ,__FILE__,__LINE__,XT_INFO);
} else {
	XT::log('please use a .csv file',__FILE__,__LINE__,XT_ERROR);
}
XT::setAdminModule('import');
?>