...
// save standard
XT::query("UPDATE " . $GLOBALS['plugin']->getTable('packaging_units') . " SET standard= '" . $GLOBALS['plugin']->getValue('standard') . "' WHERE id=" . $GLOBALS['plugin']->getValue('id'),__FILE__,__LINE__);
// save language specific changes
XT::query("DELETE FROM " . $GLOBALS['plugin']->getTable('packaging_units_det') . "
           WHERE id=" . $GLOBALS['plugin']->getValue('id') . " AND lang='" . $GLOBALS['plugin']->getValue('save_lang') . "'",__FILE__,__LINE__,0);
XT::query("INSERT INTO " . $GLOBALS['plugin']->getTable('packaging_units_det') . " (id, lang, short, full) VALUES (
