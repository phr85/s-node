...
if(XT::getPermission("edit")){
    ...
    $result = XT::query("
        SELECT
            ...
            ud.full
        FROM
            " . XT::getTable('packaging_units') . " as u
        LEFT JOIN
            " . XT::getTable('packaging_units_det') . " as ud ON (u.id = ud.id AND ud.lang='" . $GLOBALS['plugin']->getActiveLang() . "')
    ...
    $result = XT::query("
        SELECT
            ...
            ud.short
        FROM
            " . XT::getTable('packaging_units_relations') . " as r
        LEFT JOIN
            " . XT::getTable('packaging_units_det') . " as ud ON (r.relation_id = ud.id AND ud.lang='" . $GLOBALS['plugin']->getActiveLang() . "')
