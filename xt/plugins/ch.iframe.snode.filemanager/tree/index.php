...
if(!is_numeric($node_id)){
	...
	XT::setSessionValue("startnode",$node_id);
}

// explizite startnode mit parametern setzen und holen
if(XT::getParam('setStartNode')!=""){
    XT::setSessionValue(XT::getParam('setStartNode'),$opennode_id);
}
if(XT::getParam('useStartNode')!=""){
   $node_id = XT::getSessionValue(XT::getParam('useStartNode'));
}

// get node details
$result = XT::query("SELECT id, l, r, pid, level, tree_id FROM " . $GLOBALS['plugin']->getTable("files_tree") . " WHERE id='" . $node_id . "'" ,__FILE__,__LINE__);
$node = array();
...
if ($GLOBALS['plugin']->getParam("follow") == 1){
    // get follow details
    $result = XT::query("SELECT   id, l, r, pid, level, tree_id FROM " . $GLOBALS['plugin']->getTable("files_tree") . " WHERE id='" . XT::getSessionValue('node') . "'" ,__FILE__,__LINE__);
    $follow = array();
...
if(!$no_nodes){
    $result = XT::query("
            ...
            FROM
                " . $GLOBALS['plugin']->getTable("files_tree") . " AS main LEFT JOIN
                " . $GLOBALS['plugin']->getTable("files_tree_details") . " AS details ON (details.node_id = main.ID AND details.lang = '" . $lang . "')
            WHERE "
    . $sql['open_depth'] . "
    ( main.pid in (" . $in . ")"
    . $sql['start_level']
